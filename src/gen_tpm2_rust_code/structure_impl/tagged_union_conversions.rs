...
impl<'a> Tpm2InterfaceRustCodeGenerator<'a> {
    ...
    pub(in super) fn gen_tagged_union_to_discriminant<W: io::Write>(
        &self,
        ...
        let mut tagged_union_config_deps = closure_deps.collect_config_deps(ClosureDepsFlags::ANY_DEFINITION);
            ...
            match discriminant.resolved_discriminant_type.as_ref().unwrap() {
                StructureTableEntryResolvedDiscriminantType::Constants(i) => {
                    ...
                    (
                        ...
                        constants_table.closure_deps.collect_config_deps(ClosureDepsFlags::ANY_DEFINITION)
                    )
                },
        ...
        if enable_enum_transmute {
            ...
            for selector in UnionSelectorIterator::new(
                ...
                *discriminant.resolved_discriminant_type.as_ref().unwrap(),
                ...
                if discriminant.discriminated_union_members.len() == 1 {
                    writeln!(&mut iiiout, "{}::{}({}) => Self::{},",
                             &tagged_union_name, &enum_member_name,
                             selected_union_members_match_spec, &enum_member_name)?;
                } else {
                    writeln!(&mut iiiout, "{}::{}{{{}}} => Self::{},",
                             &tagged_union_name, &enum_member_name,
                             selected_union_members_match_spec, &enum_member_name)?;
    ...
    pub(super) fn gen_tagged_union_non_cond_cond_conversions<W: io::Write>(
        ...
        let (tagged_union_name_noncond, tagged_union_name_cond) = if !is_structure_member_repr {
            ...
            (
                Self::camelize(
                    &Self::format_structure_discriminant_member_enum_name(
                        table,
                        false,
                        discriminant_entry
                    )
        ...
        let error_rc_value =
            match discriminant.resolved_discriminant_type.as_ref().unwrap() {
                ...
                self.tables.structures.lookup_constant("TPM_RC_VALUE").unwrap()
            });
        ...
        for selector in UnionSelectorIterator::new(
            ...
            *discriminant.resolved_discriminant_type.as_ref().unwrap(),
            ...
            writeln!(&mut iiiiout, "if let {}::{}{}{}{} = other {{",
                     &tagged_union_name_noncond,
                     &enum_member_name,
                     match_delims.0,
